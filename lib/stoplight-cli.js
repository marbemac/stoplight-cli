...
var setEnvironment = function(authToken, app) {
  ...
  var options = {
    method: 'post',
    uri: 'https://api.stoplight.io/apps/'+app._id+'/environments',
    json: {
      authToken: authToken,
      local: true,
      name: os.hostname()
  ...
  request(options, function(error, response, body) {
    ...
    if (!environment.url) {
      ...
      prompt.get(schema, function (err, result) {
        var options = {
          method: 'put',
          uri: 'https://api.stoplight.io/environments/'+environment._id+'',
          json: {
            authToken: authToken,
        ...
        request(options, function(error, response, body) {
          console.log("Saved! You can always change this URL at https://dash.stoplight.io.".green);
...
var setApp = function(authToken) {
  var options = {
    method: 'get',
    uri: 'https://api.stoplight.io/apps',
    json: true,
    qs: {
      authToken: authToken
    }
  ...
  request(options, function(error, response, body) {
    ...
    if (!body.response.apps || body.response.apps.length == 0) {
      console.log("You have not created any apps yet. Please login at https://dash.stoplight.io and create one. After you\'ve done that, run 'stoplight app' to setup a local environment for the app.".red);
      return;
    }
...
program
  ...
  .action(function() {
    ...
    console.log('Your authToken can be found by signing in at https://dash.stoplight.io and clicking your name in the top right.'.yellow)
...
program
  ...
  .action(function(){
    ...
    var socket = new Pusher('18aa3a51daa63ece0cb9');
    ...
    channel.bind('do_request', function(requestData) {
      ...
      request(options, function(error, response, body) {
        ...
        var completeOptions = {
          method: 'put',
          uri: 'https://api.stoplight.io/requests/'+requestData._id+'/complete',
          json: {
            authToken: authToken,
            requestData: data
          }
